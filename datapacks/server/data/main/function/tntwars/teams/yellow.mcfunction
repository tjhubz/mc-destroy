...
execute if score tntwars_timer global matches 6.. run tellraw @a[team=yellow] [{selector:"@s",color:"gray",type:"selector"},{text:" joined your team.",color:"gray",type:"text"}]
team join yellow @s
tag @s add tntwars
tag @s add alive
gamemode survival @s
tp @s @e[type=minecraft:armor_stand,tag=yellowSpawn,limit=1]
execute at @e[type=minecraft:armor_stand,tag=yellowSpawn,limit=1] run spawnpoint @s ~ ~ ~ ~
