...
execute if score tntwars_timer global matches 6.. run tellraw @a[team=blue] [{selector:"@s",color:"gray",type:"selector"},{text:" joined your team.",color:"gray",type:"text"}]
team join blue @s
tag @s add tntwars
tag @s add alive
gamemode survival @s
tp @s @e[type=minecraft:armor_stand,tag=blueSpawn,limit=1]
execute at @e[type=minecraft:armor_stand,tag=blueSpawn,limit=1] run spawnpoint @s ~ ~ ~ ~
